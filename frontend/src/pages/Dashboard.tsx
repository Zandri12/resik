import { format, differenceInDays } from 'date-fns'
import { id } from 'date-fns/locale'
import { useCallback, useEffect, useRef, useState, type ReactNode } from 'react'
import { Link } from 'react-router-dom'
import { toast } from 'sonner'
import { Calendar } from '@/components/ui/calendar'
import { Popover, PopoverContent, PopoverTrigger } from '@/components/ui/popover'
import {
  Table,
  TableBody,
  TableCell,
  TableHead,
  TableHeader,
  TableRow,
} from '@/components/ui/table'
import {
  DATA_TABLE_BODY_CELL_CLASS,
  DATA_TABLE_BODY_ROW_CLASS,
  DATA_TABLE_HEADER_CELL_CLASS,
  DATA_TABLE_HEADER_ROW_CLASS,
} from '@/components/data-table'
import { TooltipContent, TooltipProvider, TooltipRoot, TooltipTrigger } from '@/components/ui/tooltip'
import { cn } from '@/lib/utils'
import { OrderCreatorDisplay, type OrderCreator } from '@/components/order/OrderCreatorDisplay'
import DashboardKaryawan, { TERMINAL_STATUS } from '../components/dashboard/DashboardKaryawan'
import { dashboardApi, ordersApi, orderStatusesApi } from '../services/api'
import { useAuth } from '../contexts/AuthContext'

type DashboardData = {
  orders_count: number
  gross_total: number
  income: number
  expenses: number
  profit: number
  period: string
  pending_orders?: number
  delivered_orders?: number
  unpaid_orders?: number
  total_paid?: number
  cash_received?: number
  income_accrual?: number
  profit_accrual?: number
  receivables?: number
  orders_by_status?: { name: string; count: number }[]
}

type TrendPoint = {
  date: string
  label: string
  orders_count: number
  income: number
  cash_received?: number
  by_status?: { name: string; count: number }[]
}

/** Satu titik untuk tren pendapatan (harian / rentang) dari weeklyTrend */
type IncomeTrendPoint = {
  date: string
  day_label: string
  orders_count: number
  income: number
  cash_received?: number
}

function trendSeriesCash(p: IncomeTrendPoint): number {
  return p.cash_received ?? p.income
}

type OrderRow = {
  id: number
  order_number: string
  total: number
  created_at?: string
  customer?: { name: string }
  status?: { name: string }
  items?: { quantity: number; service_package?: { name: string } }[]
  created_by?: OrderCreator | null
}

function extractOrderRows(res: { data: unknown }): OrderRow[] {
  const body = res.data as { data?: OrderRow[] } | OrderRow[] | null | undefined
  if (Array.isArray(body)) return body
  if (body && typeof body === 'object' && Array.isArray(body.data)) return body.data
  return []
}

const STATUS_LABEL: Record<string, string> = {
  diterima: 'Diterima',
  diproses: 'Diproses',
  selesai: 'Selesai',
  batal: 'Batal',
  diambil: 'Selesai',
  siap_diambil: 'Selesai',
  setrika: 'Diproses',
  cuci: 'Diproses',
}

/** Badge status — palet lembut (sama di Order Terbaru & Status Order Hari Ini) */
const STATUS_CLASS: Record<string, string> = {
  diterima: 'bg-palette-cream text-on-surface',
  diproses: 'bg-palette-sky text-on-surface',
  selesai: 'bg-palette-purple text-on-surface',
  batal: 'bg-palette-lavender/50 text-on-surface border border-palette-purple/35',
  cuci: 'bg-palette-sky text-on-surface',
  setrika: 'bg-palette-sky text-on-surface',
  siap_diambil: 'bg-palette-purple/80 text-on-surface',
  diambil: 'bg-palette-purple text-on-surface',
}

const INCOME_CHART_COLOR = '#9B8EC7'

/** Warna untuk stacked bar chart order per status (Tailwind bg-*) */
const CHART_STATUS_COLORS: Record<string, string> = {
  diterima: 'bg-palette-sky',
  diproses: 'bg-palette-lavender',
  selesai: 'bg-palette-cream',
  batal: 'bg-palette-purple/80',
  cuci: 'bg-palette-lavender',
  setrika: 'bg-palette-lavender',
  siap_diambil: 'bg-palette-cream',
  diambil: 'bg-palette-cream',
}

/** Urutan status di chart (sesuai backend sort_order) */
const ORDER_STATUS_CHART_ORDER = ['diterima', 'diproses', 'selesai', 'batal'] as const

function formatRupiah(n: number) {
  const s = new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 }).format(
    Math.abs(n)
  )
  return n < 0 ? `-${s}` : s
}

function SummaryStatCard({
  icon,
  iconWrapperClassName,
  label,
  subtitle,
  value,
  valueClassName = 'text-on-surface',
}: {
  icon: string
  iconWrapperClassName: string
  label: string
  subtitle?: string
  value: ReactNode
  valueClassName?: string
}) {
  return (
    <div className="bg-surface-container-lowest p-6 rounded-xl border border-transparent shadow-sm flex flex-col justify-between">
      <div className="flex items-center justify-between mb-4">
        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${iconWrapperClassName}`}>
          <span className="material-symbols-outlined">{icon}</span>
        </div>
      </div>
      <div>
        <p className="text-on-surface-variant text-sm font-medium">{label}</p>
        {subtitle ? (
          <p className="text-xs text-on-surface-variant/85 mt-0.5 font-body">{subtitle}</p>
        ) : null}
        <h3 className={`font-headline font-extrabold text-2xl mt-1 tabular-nums ${valueClassName}`}>{value}</h3>
      </div>
    </div>
  )
}

function formatShortRupiah(n: number) {
  if (n >= 1_000_000) return `Rp ${(n / 1_000_000).toFixed(0)}jt`
  if (n >= 1_000) return `Rp ${(n / 1_000).toFixed(0)}k`
  return formatRupiah(n)
}

function IncomeChartTooltipBody({ point }: { point: IncomeTrendPoint }) {
  const kas = trendSeriesCash(point)
  const nota = point.income
  return (
    <div className="space-y-1">
      <p className="font-semibold text-sm text-popover-foreground">
        {point.day_label}, {point.date}
      </p>
      <p className="tabular-nums text-popover-foreground">
        Kas masuk: <strong>{formatRupiah(kas)}</strong>
      </p>
      {point.cash_received != null && Math.abs(nota - kas) > 0.01 && (
        <p className="tabular-nums text-xs text-on-surface-variant">
          Nilai nota (masuk hari ini): {formatRupiah(nota)}
        </p>
      )}
      <p className="text-on-surface-variant">{point.orders_count} order (nota masuk)</p>
    </div>
  )
}

function MonthlyChartTooltipBody({ point }: { point: TrendPoint }) {
  const segments = (point.by_status ?? []).filter((s) => s.count > 0)
  const kas = point.cash_received
  return (
    <div className="space-y-1.5">
      <p className="font-semibold text-sm text-popover-foreground">{point.label}</p>
      {kas != null && (
        <p className="tabular-nums text-popover-foreground">
          Kas masuk: <strong>{formatRupiah(kas)}</strong>
        </p>
      )}
      <p className="tabular-nums text-popover-foreground">
        Total: <strong>{point.orders_count}</strong> order
      </p>
      {segments.length > 0 && (
        <ul className="space-y-0.5 border-t border-border pt-1.5 mt-1.5">
          {segments.map((seg) => (
            <li key={seg.name} className="flex justify-between gap-4 text-on-surface-variant">
              <span>{STATUS_LABEL[seg.name] ?? seg.name}</span>
              <span className="font-medium tabular-nums text-popover-foreground">{seg.count}</span>
            </li>
          ))}
        </ul>
      )}
    </div>
  )
}

function getServiceLabel(order: OrderRow): string {
  const items = order.items ?? []
  if (items.length === 0) return '-'
  const first = items[0]
  const name = first.service_package?.name ?? 'Layanan'
  const qty = first.quantity
  return `${name} - ${qty}kg`
}

type ChartType = 'bar' | 'line' | 'area'
type ChartMode = 'preset' | 'range'
type ChartPeriod = 'this_week' | 'last_week'
type SummaryPeriod = 'today' | 'week' | 'month'

const SUMMARY_PERIOD_LABEL: Record<SummaryPeriod, string> = {
  today: 'Hari ini',
  week: 'Minggu ini',
  month: 'Bulan ini',
}

const MONTHLY_TREND_MONTHS = 12
const MAX_RANGE_DAYS = 31

export default function Dashboard() {
  const { user } = useAuth()
  const isKaryawan = user?.role === 'karyawan'
  const [data, setData] = useState<DashboardData | null>(null)
  const [trend, setTrend] = useState<TrendPoint[]>([])
  const [incomeTrend, setIncomeTrend] = useState<IncomeTrendPoint[]>([])
  const [recentOrders, setRecentOrders] = useState<OrderRow[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [chartType, setChartType] = useState<ChartType>('bar')
  const [chartMode, setChartMode] = useState<ChartMode>('preset')
  const [chartPeriod, setChartPeriod] = useState<ChartPeriod>('this_week')
  const [chartEndDate, setChartEndDate] = useState<Date | null>(null)
  const [chartRange, setChartRange] = useState<{ from: Date | null; to: Date | null }>({ from: null, to: null })
  const [chartPopoverOpen, setChartPopoverOpen] = useState(false)
  const hasShownEmptyToast = useRef(false)
  const [workQueue, setWorkQueue] = useState<OrderRow[]>([])
  const [orderStatuses, setOrderStatuses] = useState<{ id: number; name: string; sort_order?: number }[]>([])
  const [refreshing, setRefreshing] = useState(false)
  const [summaryPeriod, setSummaryPeriod] = useState<SummaryPeriod>('today')

  const fetchDashboard = useCallback((opts?: { soft?: boolean }) => {
    const soft = opts?.soft === true
    const todayStr = format(new Date(), 'yyyy-MM-dd')
    let incomeTrendOpts: Parameters<typeof dashboardApi.weeklyTrend>[0] | undefined
    if (!isKaryawan) {
      if (chartMode === 'range' && chartRange.from && chartRange.to) {
        const days = differenceInDays(chartRange.to, chartRange.from) + 1
        if (days > MAX_RANGE_DAYS) incomeTrendOpts = undefined
        else
          incomeTrendOpts = {
            from_date: format(chartRange.from, 'yyyy-MM-dd'),
            to_date: format(chartRange.to, 'yyyy-MM-dd'),
            all_statuses: true,
          }
      } else if (chartEndDate) {
        incomeTrendOpts = { end_date: format(chartEndDate, 'yyyy-MM-dd'), all_statuses: true }
      } else {
        incomeTrendOpts = { offset: chartPeriod === 'last_week' ? 1 : 0, all_statuses: true }
      }
    }

    setError(null)
    if (soft) {
      setRefreshing(true)
    } else {
      setLoading(true)
    }

    const recentParams = isKaryawan
      ? { page: '1', per_page: '8' }
      : { page: '1', per_page: '5' }
    const recentPromise = ordersApi.list(recentParams).then(extractOrderRows)
    const todayOrdersPromise = isKaryawan
      ? ordersApi.list({ from: todayStr, to: todayStr, per_page: '50' }).then(extractOrderRows)
      : Promise.resolve([] as OrderRow[])

    const statusesPromise = isKaryawan
      ? orderStatusesApi.list().then((r) => {
          const raw = r.data as { id: number; name: string; sort_order?: number }[] | undefined
          return Array.isArray(raw) ? raw : []
        })
      : Promise.resolve([] as { id: number; name: string; sort_order?: number }[])

    const dashboardPeriod = isKaryawan ? 'today' : summaryPeriod

    Promise.all([
      dashboardApi.get(dashboardPeriod).then((r) => {
        const raw = r.data as DashboardData | { data?: DashboardData }
        const d = (raw && typeof raw === 'object' && 'data' in raw ? raw.data : raw) as DashboardData | undefined
        return {
          orders_count: Number(d?.orders_count ?? 0),
          gross_total: Number(d?.gross_total ?? d?.income ?? 0),
          income: Number(d?.income ?? 0),
          expenses: Number(d?.expenses ?? 0),
          profit: Number(d?.profit ?? 0),
          period: d?.period ?? 'today',
          pending_orders: d?.pending_orders != null ? Number(d.pending_orders) : undefined,
          delivered_orders: d?.delivered_orders != null ? Number(d.delivered_orders) : undefined,
          unpaid_orders: d?.unpaid_orders != null ? Number(d.unpaid_orders) : undefined,
          total_paid: d?.total_paid != null ? Number(d.total_paid) : undefined,
          cash_received: d?.cash_received != null ? Number(d.cash_received) : undefined,
          income_accrual: d?.income_accrual != null ? Number(d.income_accrual) : undefined,
          profit_accrual: d?.profit_accrual != null ? Number(d.profit_accrual) : undefined,
          receivables: d?.receivables != null ? Number(d.receivables) : undefined,
          orders_by_status: Array.isArray(d?.orders_by_status) ? d.orders_by_status : undefined,
        } satisfies DashboardData
      }),
      isKaryawan
        ? Promise.resolve([] as IncomeTrendPoint[])
        : incomeTrendOpts
          ? dashboardApi.weeklyTrend(incomeTrendOpts).then((r) => {
              const raw = r.data as { days?: IncomeTrendPoint[] } | { data?: { days?: IncomeTrendPoint[] } }
              const unwrapped = raw && typeof raw === 'object' && 'data' in raw ? (raw as { data?: { days?: IncomeTrendPoint[] } }).data : (raw as { days?: IncomeTrendPoint[] })
              return Array.isArray(unwrapped?.days) ? unwrapped.days : []
            })
          : Promise.resolve([] as IncomeTrendPoint[]),
      isKaryawan
        ? Promise.resolve([] as TrendPoint[])
        : dashboardApi.monthlyTrend(MONTHLY_TREND_MONTHS).then((r) => {
            const raw = r.data as { months?: { date: string; month_label: string; orders_count: number; income: number; cash_received?: number; by_status?: { name: string; count: number }[] }[] } | { data?: { months?: { date: string; month_label: string; orders_count: number; income: number; cash_received?: number; by_status?: { name: string; count: number }[] }[] } }
            const unwrapped = raw && typeof raw === 'object' && 'data' in raw ? (raw as { data?: { months?: { date: string; month_label: string; orders_count: number; income: number; cash_received?: number; by_status?: { name: string; count: number }[] }[] } }).data : (raw as { months?: { date: string; month_label: string; orders_count: number; income: number; cash_received?: number; by_status?: { name: string; count: number }[] }[] })
            const months = Array.isArray(unwrapped?.months) ? unwrapped.months : []
            return months.map((m) => ({
              date: m.date,
              label: m.month_label,
              orders_count: m.orders_count,
              income: m.income,
              cash_received: m.cash_received != null ? Number(m.cash_received) : undefined,
              by_status: Array.isArray(m.by_status) ? m.by_status : [],
            }))
          }),
      recentPromise,
      todayOrdersPromise,
      statusesPromise,
    ])
      .then(([d, incomeDays, monthly, recent, todayList, statuses]) => {
        setData(d)
        setIncomeTrend(Array.isArray(incomeDays) ? incomeDays : [])
        setTrend(Array.isArray(monthly) ? monthly : [])
        setRecentOrders(Array.isArray(recent) ? recent : [])
        if (isKaryawan) {
          setOrderStatuses(statuses)
          setWorkQueue(
            todayList.filter((o) => !TERMINAL_STATUS.has(o.status?.name ?? ''))
          )
        } else {
          setOrderStatuses([])
          setWorkQueue([])
        }
      })
      .catch((e) => {
        const msg = e?.response?.data?.message ?? e?.response?.data?.error ?? e?.message ?? 'Gagal memuat dashboard.'
        const text = typeof msg === 'string' ? msg : 'Gagal memuat dashboard.'
        if (soft) {
          toast.error(text)
        } else {
          setError(text)
        }
      })
      .finally(() => {
        setLoading(false)
        setRefreshing(false)
      })
  }, [chartMode, chartPeriod, chartEndDate, chartRange.from, chartRange.to, isKaryawan, summaryPeriod])

  useEffect(() => {
    fetchDashboard()
  }, [fetchDashboard])

  useEffect(() => {
    if (
      !loading &&
      data &&
      data.orders_count === 0 &&
      recentOrders.length === 0 &&
      !hasShownEmptyToast.current
    ) {
      hasShownEmptyToast.current = true
      toast.info(isKaryawan ? 'Belum ada order hari ini' : 'Belum ada aktivitas hari ini', {
        description: isKaryawan
          ? 'Order baru akan muncul di daftar setelah dicatat.'
          : 'Buat order pertama untuk mulai mencatat transaksi.',
      })
    }
  }, [loading, data, recentOrders.length, isKaryawan])

  if (loading && !data) {
    return (
      <div className="flex min-h-[40vh] items-center justify-center p-4 sm:p-8">
        <p className="text-on-surface-variant">Memuat dashboard...</p>
      </div>
    )
  }

  if (error && !data) {
    return (
      <div className="flex min-h-[40vh] flex-col items-center justify-center gap-4 p-4 sm:p-8">
        <p className="text-on-surface-variant text-center">{error}</p>
        <button
          type="button"
          onClick={() => fetchDashboard()}
          className="bg-primary text-on-primary px-5 py-2.5 rounded-xl font-bold text-sm hover:opacity-90 transition-all"
        >
          Coba lagi
        </button>
      </div>
    )
  }

  if (!data) return null

  const name = user?.name ?? (isKaryawan ? 'Karyawan' : 'Admin')
  const todayIso = format(new Date(), 'yyyy-MM-dd')

  if (isKaryawan) {
    return (
      <DashboardKaryawan
        name={name}
        data={data}
        recentOrders={recentOrders}
        workQueue={workQueue}
        orderStatuses={orderStatuses}
        refreshing={refreshing}
        todayIso={todayIso}
        onRefresh={() => fetchDashboard({ soft: true })}
      />
    )
  }

  const maxIncome = Math.max(...incomeTrend.map((d) => trendSeriesCash(d)), 1)

  const grossTotal = data.gross_total ?? data.income
  const unpaidOrders = data.unpaid_orders ?? 0
  const totalPaid = data.total_paid ?? 0
  const receivables = data.receivables ?? 0
  const maxOrders = Math.max(...trend.map((d) => d.orders_count), 1)
  const summaryScopeLabel = SUMMARY_PERIOD_LABEL[summaryPeriod]
  return (
    <div className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">
      {/* Welcome */}
      <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <p className="text-xs font-bold uppercase tracking-wider mb-1 text-on-surface-variant">Ringkasan bisnis</p>
          <h1 className="font-headline text-2xl font-extrabold tracking-tight text-on-surface sm:text-3xl">
            Selamat Datang, {name}
          </h1>
          <p className="text-on-surface-variant mt-1 font-body">Pantau performa bisnis laundry Anda hari ini.</p>
        </div>
        <div className="flex gap-3 flex-wrap">
          <Link
            to="/dashboard/orders"
            className="bg-surface-container-low text-primary px-5 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 hover:bg-surface-container-high transition-all"
          >
            <span className="material-symbols-outlined text-sm">list_alt</span>
            Lihat Semua Order
          </Link>
          <Link
            to="/dashboard/orders/new"
            className="bg-gradient-to-br from-primary to-primary-container text-on-primary px-6 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 shadow-lg shadow-primary/10 hover:opacity-90 transition-all"
          >
            <span className="material-symbols-outlined text-sm">add</span>
            Buat Order Baru
          </Link>
        </div>
      </div>

      {/* Summary */}
      <div className="mb-10">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
              <h2 className="font-headline font-bold text-lg text-on-surface">Ringkasan</h2>
              <div
                className="inline-flex rounded-xl border border-outline-variant/40 bg-surface-container-low p-1 gap-0.5"
                role="group"
                aria-label="Periode ringkasan"
              >
                {(['today', 'week', 'month'] as const).map((p) => (
                  <button
                    key={p}
                    type="button"
                    onClick={() => setSummaryPeriod(p)}
                    className={cn(
                      'px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors',
                      summaryPeriod === p
                        ? 'bg-primary text-primary-foreground shadow-sm'
                        : 'text-on-surface-variant hover:bg-surface-container-high'
                    )}
                  >
                    {SUMMARY_PERIOD_LABEL[p]}
                  </button>
                ))}
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <SummaryStatCard
                icon="inventory_2"
                iconWrapperClassName="bg-primary/5 text-primary"
                label="Laundry Masuk"
                subtitle={summaryScopeLabel}
                value={data.orders_count}
              />
              <SummaryStatCard
                icon="payments"
                iconWrapperClassName="bg-secondary/5 text-secondary"
                label="Nilai Transaksi"
                subtitle={summaryScopeLabel}
                value={formatShortRupiah(grossTotal)}
              />
              <SummaryStatCard
                icon="account_balance_wallet"
                iconWrapperClassName="bg-error/5 text-error"
                label="Total Pengeluaran"
                subtitle={summaryScopeLabel}
                value={formatShortRupiah(data.expenses)}
              />
              <SummaryStatCard
                icon="trending_up"
                iconWrapperClassName="bg-tertiary/5 text-tertiary"
                label="Laba (kas)"
                subtitle={summaryScopeLabel}
                value={
                  data.profit_accrual != null && Math.abs(data.profit_accrual - data.profit) > 0.01 ? (
                    <span className="block">
                      <span className="block">{formatShortRupiah(data.profit)}</span>
                      <span className="block text-xs font-normal text-on-surface-variant mt-1 font-body">
                        Akrual: {formatShortRupiah(data.profit_accrual)}
                      </span>
                    </span>
                  ) : (
                    formatShortRupiah(data.profit)
                  )
                }
                valueClassName="text-primary"
              />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
              <SummaryStatCard
                icon="schedule"
                iconWrapperClassName="bg-secondary/5 text-secondary"
                label="Laundry belum bayar"
                subtitle={summaryScopeLabel}
                value={unpaidOrders}
              />
              <SummaryStatCard
                icon="price_check"
                iconWrapperClassName="bg-secondary/5 text-secondary"
                label="Kas masuk"
                subtitle={summaryScopeLabel}
                value={formatShortRupiah(totalPaid)}
              />
              <SummaryStatCard
                icon="receipt_long"
                iconWrapperClassName="bg-tertiary/5 text-tertiary"
                label="Piutang (belum lunas)"
                subtitle="Total semua order"
                value={formatShortRupiah(receivables)}
              />
            </div>
      </div>

      {/* Chart */}
      <div className="mb-10 overflow-hidden rounded-xl bg-surface-container-lowest p-4 shadow-sm sm:p-6 lg:p-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            <h2 className="font-headline font-bold text-lg">Tren Kas Masuk</h2>
            <p className="text-xs text-on-surface-variant">
              Kas masuk (paid_at) per hari; batang mengikuti kas — jumlah order = nota yang dibuat hari itu
            </p>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex items-center gap-2 mr-2">
              <span className="w-3 h-3 rounded-full bg-palette-purple" />
              <span className="text-xs font-medium text-on-surface-variant">Kas masuk</span>
            </div>
            <Popover open={chartPopoverOpen} onOpenChange={setChartPopoverOpen}>
              <PopoverTrigger
                className="text-xs bg-surface border border-outline-variant/40 rounded-lg cursor-pointer py-1.5 pl-3 pr-8 hover:bg-surface-container-low min-h-8 inline-flex items-center gap-1"
              >
                {chartMode === 'range' && chartRange.from
                  ? chartRange.to
                    ? `${format(chartRange.from, 'd MMM', { locale: id })} - ${format(chartRange.to, 'd MMM yyyy', { locale: id })}`
                    : `${format(chartRange.from, 'd MMM', { locale: id })} - ...`
                  : chartEndDate
                    ? format(chartEndDate, 'd MMM yyyy', { locale: id })
                    : chartPeriod === 'last_week'
                      ? 'Minggu Lalu'
                      : 'Minggu Ini'}
                <span className="material-symbols-outlined text-sm ml-0.5">expand_more</span>
              </PopoverTrigger>
              <PopoverContent className="p-0">
                <div className="p-2 border-b border-border flex gap-1">
                  <button
                    type="button"
                    className={`text-xs px-3 py-1.5 rounded-md ${chartMode === 'preset' ? 'bg-primary text-primary-foreground' : 'bg-surface-container-low hover:bg-surface-container-high'}`}
                    onClick={() => {
                      setChartMode('preset')
                      setChartRange({ from: null, to: null })
                    }}
                  >
                    7 hari
                  </button>
                  <button
                    type="button"
                    className={`text-xs px-3 py-1.5 rounded-md ${chartMode === 'range' ? 'bg-primary text-primary-foreground' : 'bg-surface-container-low hover:bg-surface-container-high'}`}
                    onClick={() => {
                      setChartMode('range')
                      setChartEndDate(null)
                      setChartPeriod('this_week')
                    }}
                  >
                    Rentang
                  </button>
                </div>
                {chartMode === 'preset' ? (
                  <>
                    <Calendar
                      mode="single"
                      selected={chartEndDate ?? undefined}
                      onSelect={(date) => {
                        if (date) {
                          setChartEndDate(date)
                          setChartPopoverOpen(false)
                        }
                      }}
                      locale={id}
                      defaultMonth={chartEndDate ?? new Date()}
                    />
                    <div className="p-2 border-t border-border flex gap-2">
                      <button
                        type="button"
                        className="text-xs px-2 py-1.5 rounded-md bg-surface-container-low hover:bg-surface-container-high"
                        onClick={() => {
                          setChartEndDate(null)
                          setChartPeriod('this_week')
                          setChartPopoverOpen(false)
                        }}
                      >
                        Minggu Ini
                      </button>
                      <button
                        type="button"
                        className="text-xs px-2 py-1.5 rounded-md bg-surface-container-low hover:bg-surface-container-high"
                        onClick={() => {
                          setChartEndDate(null)
                          setChartPeriod('last_week')
                          setChartPopoverOpen(false)
                        }}
                      >
                        Minggu Lalu
                      </button>
                    </div>
                  </>
                ) : (
                  <div className="p-2">
                    <Calendar
                      mode="range"
                      selected={
                        chartRange.from || chartRange.to
                          ? { from: chartRange.from ?? undefined, to: chartRange.to ?? undefined }
                          : undefined
                      }
                      onSelect={(range) => {
                        if (!range) return
                        const from = range.from ?? null
                        const to = range.to ?? null
                        setChartRange({ from, to })
                        if (from && to) {
                          const days = differenceInDays(to, from) + 1
                          if (days > MAX_RANGE_DAYS) {
                            toast.error('Maksimal 31 hari', { description: 'Pilih rentang yang lebih pendek.' })
                            return
                          }
                          setChartPopoverOpen(false)
                        }
                      }}
                      locale={id}
                      defaultMonth={chartRange.from ?? chartRange.to ?? new Date()}
                    />
                    <p className="text-[10px] text-on-surface-variant mt-2">Maks. 31 hari</p>
                  </div>
                )}
              </PopoverContent>
            </Popover>
            <div className="flex items-center gap-0.5 rounded-lg border border-outline-variant/40 bg-surface overflow-hidden">
              {(['bar', 'line', 'area'] as const).map((type) => (
                <button
                  key={type}
                  type="button"
                  title={type === 'bar' ? 'Batang' : type === 'line' ? 'Garis' : 'Area'}
                  className={`p-2 transition-colors ${chartType === type ? 'bg-primary text-primary-foreground' : 'hover:bg-surface-container-low text-on-surface-variant'}`}
                  onClick={() => setChartType(type)}
                >
                  <span className="material-symbols-outlined text-lg">
                    {type === 'bar' ? 'bar_chart' : type === 'line' ? 'show_chart' : 'area_chart'}
                  </span>
                </button>
              ))}
            </div>
          </div>
        </div>
        <div className="h-64 flex flex-col relative">
          <TooltipProvider>
            {(chartType === 'line' || chartType === 'area') && incomeTrend.length > 0 ? (
              <>
                <div className="flex-1 min-h-0 w-full relative px-2">
                  <div className="absolute inset-x-0 top-0 z-0 h-full flex flex-col justify-between pointer-events-none opacity-5">
                    {[0, 1, 2, 3, 4].map((i) => (
                      <div key={i} className="w-full border-t border-on-surface" />
                    ))}
                  </div>
                  <svg
                    viewBox="0 0 100 100"
                    preserveAspectRatio="none"
                    className="pointer-events-none absolute inset-0 z-0 h-full w-full"
                  >
                    {(() => {
                      const n = incomeTrend.length
                      const points = incomeTrend.map((d, i) => {
                        const x = ((i + 0.5) / n) * 100
                        const y = 100 - (maxIncome > 0 ? (trendSeriesCash(d) / maxIncome) * 100 : 0)
                        return { ...d, x, y }
                      })
                      const linePoints = points.map((p) => `${p.x},${p.y}`).join(' ')
                      const areaPoints = `0,100 ${linePoints} 100,100`
                      return (
                        <>
                          {chartType === 'area' && (
                            <polygon
                              points={areaPoints}
                              fill={INCOME_CHART_COLOR}
                              fillOpacity={0.2}
                              className="transition-opacity"
                            />
                          )}
                          <polyline
                            points={linePoints}
                            fill="none"
                            stroke={INCOME_CHART_COLOR}
                            strokeWidth={2}
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            vectorEffect="non-scaling-stroke"
                            className="transition-opacity"
                          />
                        </>
                      )
                    })()}
                  </svg>
                  <div className="absolute inset-0 z-[1] flex items-stretch">
                    {incomeTrend.map((point) => (
                      <TooltipRoot key={point.date}>
                        <TooltipTrigger
                          delay={80}
                          render={(props) => (
                            <div
                              {...props}
                              className={cn(
                                'h-full min-w-0 flex-1 cursor-default outline-none',
                                props.className
                              )}
                              aria-label={`${point.day_label}, ${point.date}: kas ${formatRupiah(trendSeriesCash(point))}, ${point.orders_count} order`}
                            />
                          )}
                        />
                        <TooltipContent side="top">
                          <IncomeChartTooltipBody point={point} />
                        </TooltipContent>
                      </TooltipRoot>
                    ))}
                  </div>
                </div>
                <div className="flex flex-shrink-0 px-2 pt-3">
                  {incomeTrend.map((point) => (
                    <span
                      key={point.date}
                      className="text-[10px] font-bold text-on-surface-variant truncate flex-1 min-w-0 text-center"
                    >
                      {point.day_label}
                    </span>
                  ))}
                </div>
              </>
            ) : (
              <div className="flex flex-1 min-h-0 items-end justify-between gap-1 px-2 lg:gap-2">
                <div className="pointer-events-none absolute inset-x-0 bottom-0 flex h-full flex-col justify-between opacity-5">
                  {[0, 1, 2, 3, 4].map((i) => (
                    <div key={i} className="w-full border-t border-on-surface" />
                  ))}
                </div>
                {incomeTrend.length > 0 ? (
                  incomeTrend.map((point) => {
                    const pct = maxIncome > 0 ? (trendSeriesCash(point) / maxIncome) * 100 : 0
                    return (
                      <TooltipRoot key={point.date}>
                        <TooltipTrigger
                          delay={80}
                          render={(props) => (
                            <div
                              {...props}
                              className={cn(
                                'group flex w-full min-w-0 flex-1 cursor-default flex-col items-center gap-3 outline-none',
                                props.className
                              )}
                              aria-label={`${point.day_label}, ${point.date}: kas ${formatRupiah(trendSeriesCash(point))}, ${point.orders_count} order`}
                            >
                              <div className="relative flex h-32 min-h-[40px] w-full max-w-16 flex-col justify-end rounded-t-lg bg-palette-purple/20 transition-all group-hover:bg-palette-purple/40">
                                <div
                                  className="absolute bottom-0 w-full min-h-[4px] rounded-t-lg bg-palette-purple"
                                  style={{ height: `${Math.max(pct, 4)}%` }}
                                />
                              </div>
                              <span className="w-full truncate text-center text-[10px] font-bold text-on-surface-variant">
                                {point.day_label}
                              </span>
                            </div>
                          )}
                        />
                        <TooltipContent side="top">
                          <IncomeChartTooltipBody point={point} />
                        </TooltipContent>
                      </TooltipRoot>
                    )
                  })
                ) : (
                  Array.from({ length: 7 }).map((_, i) => (
                    <div key={i} className="group flex w-full min-w-0 flex-1 flex-col items-center gap-3">
                      <div className="relative flex h-32 min-h-[40px] w-full max-w-16 flex-col justify-end rounded-t-lg bg-palette-purple/20">
                        <div
                          className="absolute bottom-0 w-full min-h-[4px] rounded-t-lg bg-palette-purple"
                          style={{ height: '40%' }}
                        />
                      </div>
                      <span className="text-[10px] font-bold text-on-surface-variant">-</span>
                    </div>
                  ))
                )}
              </div>
            )}
          </TooltipProvider>
        </div>
      </div>

      <div className="mb-10 overflow-hidden rounded-xl bg-surface-container-lowest p-4 shadow-sm sm:p-6 lg:p-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            <h2 className="font-headline font-bold text-lg">Tren Order Bulanan per Status</h2>
            <p className="text-xs text-on-surface-variant">
              Jumlah order per bulan per status — {MONTHLY_TREND_MONTHS} bulan terakhir
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
            {ORDER_STATUS_CHART_ORDER.map((key) => (
              <div key={key} className="flex items-center gap-1.5">
                <span
                  className={`w-3 h-3 rounded-full shrink-0 ${CHART_STATUS_COLORS[key] ?? 'bg-outline-variant'}`}
                />
                <span className="text-[10px] font-medium text-on-surface-variant">{STATUS_LABEL[key] ?? key}</span>
              </div>
            ))}
          </div>
        </div>
        <div className="h-64 flex flex-col relative">
          <TooltipProvider>
            <div className="flex flex-1 min-h-0 items-end justify-between gap-1 px-2 lg:gap-2">
              <div className="pointer-events-none absolute inset-x-0 bottom-0 flex h-full flex-col justify-between opacity-5">
                {[0, 1, 2, 3, 4].map((i) => (
                  <div key={i} className="w-full border-t border-on-surface" />
                ))}
              </div>
              {trend.length > 0 ? (
                trend.map((point) => {
                  const barHeightPct = maxOrders > 0 ? (point.orders_count / maxOrders) * 100 : 0
                  const segments = point.by_status ?? []
                  const total = point.orders_count || 1
                  const ariaParts = [
                    `${point.label}: ${point.orders_count} order`,
                    ...segments.filter((s) => s.count > 0).map(
                      (s) => `${STATUS_LABEL[s.name] ?? s.name} ${s.count}`
                    ),
                  ]
                  return (
                    <TooltipRoot key={point.date}>
                      <TooltipTrigger
                        delay={80}
                        render={(props) => (
                          <div
                            {...props}
                            className={cn(
                              'group flex w-full min-w-0 flex-1 cursor-default flex-col items-center gap-3 outline-none',
                              props.className
                            )}
                            aria-label={ariaParts.join(', ')}
                          >
                            <div
                              className="relative flex h-32 min-h-[40px] w-full max-w-16 flex-col justify-end overflow-hidden rounded-t-lg transition-all group-hover:opacity-90"
                              style={{ minHeight: 128 }}
                            >
                              <div
                                className="flex min-h-[4px] w-full flex-col-reverse rounded-t-lg"
                                style={{ height: `${Math.max(barHeightPct, 4)}%` }}
                              >
                                {segments.map((seg) => {
                                  const segPct = total > 0 ? (seg.count / total) * 100 : 0
                                  if (segPct <= 0) return null
                                  return (
                                    <div
                                      key={seg.name}
                                      className={`min-h-[2px] w-full ${CHART_STATUS_COLORS[seg.name] ?? 'bg-outline-variant'}`}
                                      style={{ height: `${segPct}%` }}
                                    />
                                  )
                                })}
                              </div>
                            </div>
                            <span className="w-full truncate text-center text-[10px] font-bold text-on-surface-variant">
                              {point.label}
                            </span>
                          </div>
                        )}
                      />
                      <TooltipContent side="top">
                        <MonthlyChartTooltipBody point={point} />
                      </TooltipContent>
                    </TooltipRoot>
                  )
                })
              ) : (
              Array.from({ length: MONTHLY_TREND_MONTHS }).map((_, i) => (
                <div key={i} className="flex flex-col items-center gap-3 w-full group flex-1 min-w-0">
                  <div className="w-full max-w-16 bg-outline-variant/20 rounded-t-lg h-32 relative flex flex-col justify-end min-h-[40px]">
                    <div className="absolute bottom-0 w-full bg-outline-variant/40 rounded-t-lg min-h-[4px]" style={{ height: '40%' }} />
                  </div>
                  <span className="text-[10px] font-bold text-on-surface-variant">-</span>
                </div>
              ))
            )}
            </div>
          </TooltipProvider>
        </div>
      </div>

      {/* Table + Status */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-surface-container-lowest rounded-xl shadow-sm overflow-hidden">
          <div className="p-6 flex items-center justify-between">
            <h2 className="font-headline font-bold text-lg">Order Terbaru</h2>
            <Link to="/dashboard/orders" className="text-primary text-sm font-bold hover:underline">
              Semua Order
            </Link>
          </div>
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow className={DATA_TABLE_HEADER_ROW_CLASS}>
                  <TableHead className={DATA_TABLE_HEADER_CELL_CLASS}>Customer</TableHead>
                  <TableHead className={DATA_TABLE_HEADER_CELL_CLASS}>Input order</TableHead>
                  <TableHead className={DATA_TABLE_HEADER_CELL_CLASS}>Layanan</TableHead>
                  <TableHead className={DATA_TABLE_HEADER_CELL_CLASS}>Status</TableHead>
                  <TableHead className={DATA_TABLE_HEADER_CELL_CLASS}>Total</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {recentOrders.length === 0 ? (
                  <TableRow>
                    <TableCell
                      colSpan={5}
                      className={`${DATA_TABLE_BODY_CELL_CLASS} py-8 text-center text-on-surface-variant text-sm`}
                    >
                      Belum ada order.
                    </TableCell>
                  </TableRow>
                ) : (
                  recentOrders.map((o) => (
                    <TableRow key={o.id} className={DATA_TABLE_BODY_ROW_CLASS}>
                      <TableCell className={`${DATA_TABLE_BODY_CELL_CLASS} text-sm`}>
                        <Link to={`/dashboard/orders/${o.id}`} className="block">
                          <p className="font-bold">{o.customer?.name ?? '-'}</p>
                          <p className="text-xs text-on-surface-variant">{o.order_number}</p>
                        </Link>
                      </TableCell>
                      <TableCell className={`${DATA_TABLE_BODY_CELL_CLASS} text-sm`}>
                        <OrderCreatorDisplay creator={o.created_by} compact className="max-w-44 min-w-0" />
                      </TableCell>
                      <TableCell className={`${DATA_TABLE_BODY_CELL_CLASS} text-sm`}>
                        {getServiceLabel(o)}
                      </TableCell>
                      <TableCell className={DATA_TABLE_BODY_CELL_CLASS}>
                        <span
                          className={`px-3 py-1 rounded-full text-[11px] font-bold ${
                            STATUS_CLASS[o.status?.name ?? ''] ?? 'bg-palette-cream text-on-surface-variant'
                          }`}
                        >
                          {STATUS_LABEL[o.status?.name ?? ''] ?? o.status?.name ?? '-'}
                        </span>
                      </TableCell>
                      <TableCell className={`${DATA_TABLE_BODY_CELL_CLASS} text-sm font-bold`}>
                        {formatRupiah(Number(o.total))}
                      </TableCell>
                    </TableRow>
                  ))
                )}
              </TableBody>
            </Table>
          </div>
        </div>
        <div className="bg-surface-container-lowest rounded-xl shadow-sm p-6">
          <h2 className="font-headline font-bold text-lg mb-6">Status Order Hari Ini</h2>
          <div className="space-y-3">
            {(data.orders_by_status ?? []).length > 0 ? (
              ORDER_STATUS_CHART_ORDER.map((statusKey) => {
                const item = data.orders_by_status?.find((s) => s.name === statusKey)
                const count = item?.count ?? 0
                const label = STATUS_LABEL[statusKey] ?? statusKey
                return (
                  <div
                    key={statusKey}
                    className="flex items-center justify-between gap-3 py-2 px-3 rounded-lg bg-surface-container-low/50"
                  >
                    <span
                      className={`text-xs font-bold px-2.5 py-1 rounded-full ${STATUS_CLASS[statusKey] ?? 'bg-palette-cream text-on-surface-variant'}`}
                    >
                      {label}
                    </span>
                    <span className="text-sm font-bold text-on-surface-variant tabular-nums">
                      {count} order
                    </span>
                  </div>
                )
              })
            ) : (
              ORDER_STATUS_CHART_ORDER.map((statusKey) => (
                <div
                  key={statusKey}
                  className="flex items-center justify-between gap-3 py-2 px-3 rounded-lg bg-surface-container-low/50"
                >
                  <span
                    className={`text-xs font-bold px-2.5 py-1 rounded-full ${STATUS_CLASS[statusKey] ?? 'bg-palette-cream text-on-surface-variant'}`}
                  >
                    {STATUS_LABEL[statusKey] ?? statusKey}
                  </span>
                  <span className="text-sm font-bold text-on-surface-variant tabular-nums">0 order</span>
                </div>
              ))
            )}
          </div>
        </div>
      </div>
    </div>
  )
}
