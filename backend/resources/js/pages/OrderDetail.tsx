            ...
            <section className="rounded-xl border border-outline-variant/20 bg-surface-container-lowest p-4 shadow-sm sm:p-8">
              ...
              <div className="space-y-4 mb-8">
                {items.map((it, i) => (
                  <div
                    key={i}
                    className="flex flex-col gap-1 border-b border-outline-variant/20 py-3 last:border-0 sm:flex-row sm:items-center sm:justify-between sm:gap-3"
                  >
                    <span className="min-w-0 break-words text-on-surface font-medium">
                      {it.service_package?.name ?? 'Layanan'} · {it.quantity}{' '}
                      {it.service_package?.unit ?? 'pcs'}
                    </span>
                    <span className="shrink-0 font-semibold text-on-surface tabular-nums sm:text-right">
                ...
                <div className="space-y-4">
                  {items.map((it, i) => {
                    ...
                    return (
                      <div
                        key={i}
                        className="flex flex-col gap-0.5 text-on-surface-variant sm:flex-row sm:items-center sm:justify-between sm:gap-3"
                      >
