    ...
    outletSettingsApi
      ...
      .then((r) => {
        ...
        setForm((f) => ({
          ...
          phone: data?.phone ?? '',
          landing_hero_url: data?.landing_hero_url ?? '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await outletSettingsApi.update([
        ...
        { key: 'landing_hero_url', value: form.landing_hero_url },
    ...
    <div className="mx-auto w-full min-w-0 max-w-2xl space-y-6 p-4 sm:p-6 lg:p-8">
      <div className="rounded-xl bg-surface-container-lowest p-4 shadow-sm sm:p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-on-surface-variant mb-1">
              Gambar hero beranda (opsional)
            </label>
            <input
              type="text"
              value={form.landing_hero_url}
              onChange={(e) => setForm((f) => ({ ...f, landing_hero_url: e.target.value }))}
              className="w-full bg-surface-container-low border border-outline-variant/30 rounded-xl px-4 py-3 text-on-surface focus:ring-2 focus:ring-primary/20 focus:border-primary/50"
              placeholder="https://… atau path relatif, mis. uploads/hero.jpg"
            />
            <p className="text-xs text-on-surface-variant mt-1">
              URL lengkap (https) atau file di disk publik (tanpa <code className="text-xs">..</code>).
              Contoh: <code className="text-xs">hero.jpg</code> jika ada di{' '}
              <code className="text-xs">storage/app/public</code> dan symlink aktif. Kosongkan untuk
              memakai variabel <code className="text-xs">VITE_LANDING_HERO_IMAGE</code> atau gambar
              bawaan.
            </p>
          </div>
          <button
